namespace MCCCivitasBlackTech
{
    ...
    {		
        ...
        static public int GetHtml(string path,ref string url,string cookieContainer)
        {
            url = string.Empty;
            int i = 0;
            do
		...
		static public string CutKeepHead(string source, string cut)
        {
            return source.Substring(source.IndexOf(cut));
        }

        static public string CutHead(string source, string cut)
        {
            return source.Substring(source.IndexOf(cut) + cut.Length);
        }

        static public string CutTail(string source, string cut)
        {
            return source.Substring(0, source.IndexOf(cut));
        }

        static public string CutBetween(string source, string begin, string end)
        {
