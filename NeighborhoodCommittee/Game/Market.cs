namespace MCCCivitasBlackTech
{
	...
	public class Market
	{
		...
		public List<Commodity> SeeCommodities(string cookieContainer)
		{
			...
			Commodities.Clear();
			if (UrlHelpers.GetHtml("http://civitas.soobb.com/Markets/Commodities/?SupplyID=" + SupplyId + "&SupplyLevel=" + SupplyLevel, ref text, cookieContainer) == 1)
			{
				...
				do
				{
					...
					Commodities.Add(new Commodity(id, name, 1));
					Commodities.Add(new Commodity(id, name, 2));
			...
			foreach (Commodity c in Commodities)
			{
				...
				if (UrlHelpers.GetHtml(
					"http://m.civitas.soobb.com/Markets/Commodities/?InventoryID=20708&SupplyID=" + SupplyId + "&DemandID=" + c.ID + "&SupplyLevel=" + SupplyLevel + "&DemandLevel=" + c.Level,
					ref text,
					cookieContainer) == 1)
				{
			...
			//clear 
			for (int i = 0, j = Commodities.Count; i < j; i++)
			{
				if (Commodities[i].BusinessCount() == 0)
				{
		...
		public void Print()
		{
			...
			foreach (Commodity c in Commodities)
			{
				...
				Console.WriteLine(
					string.Format("{0,11}", (c.Name + c.Level + "星\t")) +
